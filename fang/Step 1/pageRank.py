...
def pageRank(filename, alpha, sub_count=0, draw=False, max_iter=100000000):
    ...
    plt.clf()
    plt.scatter(out_degree.values(),in_degree.values(),s=1,c=color)
    plt.xlabel("in_degree")
    plt.ylabel("out_degree")
    # plt.colorbar()
    plt.savefig("heat_map/heat_map_" + str(alpha) + "_" + str(sub_count) + '.png',dpi=1000)

    plt.clf()
    plt.scatter(out_degree.values(),scores)
    plt.xlabel("in_degree")
    plt.ylabel("scores")
    plt.savefig("in_degree/in_degree_" + str(alpha) + "_" + str(sub_count) + '.png', dpi=1000)
    ...
    if draw or (500 >= sub_count > 0):
        plt.clf()
        nx.draw(G, node_size=50)
        plt.savefig('music_network_'+ str(alpha) + "_" + str(sub_count) +'.png',dpi=1000)

def pageRanksub(filename, alpha, sub_id, draw=False, max_iter=100000000):
    csv_data = pd.read_csv(filename)
    csv_df = pd.DataFrame(csv_data)
    G = nx.DiGraph()
    for i in range(len(csv_df)):
        if str(csv_df["influencer_id"][i])==str(sub_id) or str(csv_df["follower_id"][i])==str(sub_id):
            head = str(csv_df["influencer_id"][i]) + "***" + str(csv_df["influencer_name"][i])
            tail = str(csv_df["follower_id"][i]) + "***" + str(csv_df["follower_name"][i])
            G.add_edge(tail, head)
    ...
    pr_sort = sorted(pr.items(), key=lambda kv: (kv[1], kv[0]), reverse=True)
    # print(pr)
    with open("page_rank_result_sub/influence_page_rank_" + str(alpha) + "_" + str(sub_id) + ".csv", 'w', encoding='utf-8',
              newline='') as f:
        writer = csv.writer(f, dialect='excel')
        writer.writerow(['id', 'name', 'influence_score', 'rank', 'out_degree', 'in_degree'])
        max_value = max(pr.values())
    ...
    plt.ylabel("out_degree")
    # plt.colorbar()
    plt.savefig("heat_map_sub/heat_map_" + str(alpha) + "_" + str(sub_id) + '.png',dpi=1000)

    plt.clf()
    plt.scatter(out_degree.values(),scores)
    plt.xlabel("in_degree")
    plt.ylabel("scores")
    plt.savefig("in_degree_sub/in_degree_" + str(alpha) + "_" + str(sub_id) + '.png', dpi=1000)

    plt.clf()
    plt.scatter(in_degree.values(),scores)
    plt.xlabel("out_degree")
    plt.ylabel("scores")
    plt.savefig("out_degree_sub/out_degree_" + str(alpha) + "_" + str(sub_id) + '.png', dpi=1000)

    if draw or (500 >= sub_count > 0):
        plt.clf()
        nx.draw(G, node_size=50)
        plt.savefig('music_network_sub/music_network_'+ str(alpha) + "_" + str(sub_id) +'.png',dpi=1000)
...
if __name__ == '__main__':
    ...
    for i in range(21):
        ...
        print(alpha)
        pageRanksub(filename, alpha, 13511, True)
